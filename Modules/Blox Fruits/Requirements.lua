...
local Animation = LocalPlayer.Character.Animate;
...
--- Tween System ---
function TweenPlayer(Pos, DestroyHolder)
    ...
    spawn(function()
        game:GetService("RunService").Heartbeat:Connect(function()
            if getgenv().Tweening then
                if not game:GetService("Workspace"):FindFirstChild("AirBlock") then
                    local AirBlock = Instance.new("Part")
                    AirBlock.Name = "AirBlock"
                    AirBlock.Parent = game.Workspace
                    AirBlock.Anchored = true
                    AirBlock.Transparency = 0
                    AirBlock.Size = Vector3.new(15, 2, 15)
                elseif game:GetService("Workspace"):FindFirstChild("AirBlock") then
                    game.Workspace["AirBlock"].CFrame = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(0, -4, 0)
                end
            else
                if game:GetService("Workspace"):FindFirstChild("AirBlock") then
                    game:GetService("Workspace"):FindFirstChild("AirBlock"):Destroy()
                end
            end
        end)
    end)
    ...
    NoClip = RunService.Stepped:Connect(noClip)
    ...
    spawn(function()
        while true do
            if getgenv().Tweening then
                if (rootPart.Position - cframe.Position).Magnitude < 80 then
                    rootPart.CFrame = Pos
                end
            end
            task.wait()
