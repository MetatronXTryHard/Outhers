...
local ESP = {
    ...
    ToolColor = Color3.fromRGB(105, 0, 255),
    FaceCamera = true,
    Names = false,
    Distance = false,
    Health = false,
...
function ESP:GetColor(obj)
    ...
    local p = self:GetLocalPlayerFromChar(obj)
    return p and self.TeamColor and p.Team and p.Team.TeamColor.Color or self.Color
end

function ESP:GetLocalPlayerFromChar(char)
    local ov = self.Overrides.GetLocalPlayerFromChar
    if ov then
        return ov(char)
    end
...
function boxBase:Update()
    ...
    local PlayerDistance = math.floor((cf.p - Camera.CFrame.p).magnitude)
